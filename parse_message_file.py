...
def validate_non_negative_integer(value, key, line_number):
    ...
    try:
        ...
        if int_value < 0:
            raise ValueError(
                "Negative value '{0}' for key '{1}' on line {2}".format(
                    value, key, line_number))
        return int_value
    except ValueError as e:
        raise ValueError(
            "Invalid integer '{0}' for key '{1}' on line {2}".format(
                value, key, line_number))
...
def parse_lines(lines, validate_only=False, verbose=False):
    ...
    def parse_include_messages(file_list):
        messages = []
        for include_file in file_list:
            included_messages = parse_file(
                include_file, validate_only, verbose)
    ...
    def parse_include_categories(file_list):
        categories = []
        for include_file in file_list:
            included_categories = parse_file(
                include_file, validate_only, verbose)
    ...
    try:
        for line_number, line in enumerate(lines, 1):
            ...
            if line == "--- Include Service Start ---":
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting include service section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending include service section)".format(
                            line_number, line))
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Including file for service)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting include users section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending include users section)".format(
                            line_number, line))
                if current_service:
                    current_service['Users'].update(
                        parse_include_users(include_files))
                continue
            elif in_section['include_users']:
                include_files.append(line)
                if verbose:
                    print(
                        "Line {0}: {1} (Including file for users)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting include messages section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending include messages section)".format(
                            line_number, line))
                if current_service:
                    current_service['MessageThreads'].extend(
                        parse_include_messages(include_files))
                continue
            elif in_section['include_messages']:
                include_files.append(line)
                if verbose:
                    print(
                        "Line {0}: {1} (Including file for messages)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting include categories section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending include categories section)".format(
                            line_number, line))
                if current_service:
                    current_service['Categories'].extend(
                        parse_include_categories(include_files))
                    for category in current_service['Categories']:
                        kind_split = category.get('Kind', '').split(",")
                        category['Type'] = kind_split[0].strip() if len(
                            kind_split) > 0 else ""
                        category['Level'] = kind_split[1].strip() if len(
                            kind_split) > 1 else ""
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Including file for categories)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting new archive service)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending archive service)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting comment section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending comment section)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting category list)".format(
                            line_number, line))
                continue
                ...
                if current_category:
                    ...
                    current_category['Type'] = kind_split[0].strip() if len(
                        kind_split) > 0 else ""
                    current_category['Level'] = kind_split[1].strip() if len(
                        kind_split) > 1 else ""
                    if current_category['Type'] not in categorization_values:
                        raise ValueError(
                            "Invalid 'Type' value '{0}' on line {1}. Expected one of {2}.".format(
                                current_category['Type'], line_number, categorization_values.keys()))
                    if current_category['InSub'] != 0 and current_category['InSub'] not in category_ids[current_category['Type']]:
                        raise ValueError(
                            "InSub value '{0}' on line {1} does not match any existing ID values.".format(
                                current_category['InSub'], line_number))
                    current_service['Categories'].append(current_category)
                    category_ids[current_category['Type']].add(
                        current_category['ID'])
                current_category = None
                if verbose:
                    print(
                        "Line {0}: {1} (Ending category list)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting categorization list)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending categorization list)".format(
                            line_number, line))
                ...
                if current_service:
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting info body)".format(
                                line_number, line))
                continue
                ...
                if current_service and current_info is not None:
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending info body)".format(
                                line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting poll list)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending poll list)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Starting poll body)".format(
                            line_number, line))
                continue
                ...
                if verbose:
                    print(
                        "Line {0}: {1} (Ending poll body)".format(
                            line_number, line))
                continue
                ...
                key, value = parse_line(line)
                if key == "Entry":
                    current_service['Entry'] = validate_non_negative_integer(
                        value, "Entry", line_number)
                elif key == "Service":
                    current_service['Service'] = value
                elif key == "Categories":
                    current_service['Categorization']['Categories'] = [
                        category.strip() for category in value.split(",")]
                    if verbose:
                        print(
                            "Line {0}: Categories set to {1}".format(
                                line_number,
                                current_service['Categorization']['Categories']))
                elif key == "Forums":
                    current_service['Categorization']['Forums'] = [
                        forum.strip() for forum in value.split(",")]
                    if verbose:
                        print(
                            "Line {0}: Forums set to {1}".format(
                                line_number,
                                current_service['Categorization']['Forums']))
                    ...
                    if key == "Kind":
                        ...
                        current_category['ID'] = validate_non_negative_integer(
                            value, "ID", line_number)
                    elif key == "InSub":
                        current_category['InSub'] = validate_non_negative_integer(
                            value, "InSub", line_number)
                    elif key == "Headline":
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting user list)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending user list)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting user info)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending user info)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting message list)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending message list)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting message thread)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending message thread)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Starting message post)".format(
                                line_number, line))
                    continue
                    ...
                    if verbose:
                        print(
                            "Line {0}: {1} (Ending message post)".format(
                                line_number, line))
                    continue
                elif in_section['message_list'] and key == "Interactions":
                    current_service['Interactions'] = [
                        interaction.strip() for interaction in value.split(",")]
                    if verbose:
                        print("Line {0}: Interactions set to {1}".format(
                            line_number, current_service['Interactions']))
                elif in_section['message_list'] and key == "Status":
                    current_service['Status'] = [status.strip()
                                                 for status in value.split(",")]
                    if verbose:
                        print(
                            "Line {0}: Status set to {1}".format(
                                line_number, current_service['Status']))
                elif key == "Info":
                    current_info = []
                    in_section['info_body'] = True
                    if verbose:
                    ...
                    if key == "User":
                        user_id = validate_non_negative_integer(
                            value, "User", line_number)
                        current_service['Users'][user_id] = {'Bio': ""}
                        if verbose:
                            print(
                                "Line {0}: User ID set to {1}".format(
                                    line_number, user_id))
                    elif key == "Name":
                        ...
                        if user_id is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Location set to {1}".format(
                                        line_number, value))
                    elif key == "Joined":
                        if user_id is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Joined date set to {1}".format(
                                        line_number, value))
                    elif key == "Birthday":
                        if user_id is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Birthday set to {1}".format(
                                        line_number, value))
                        ...
                        if user_id is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Starting bio body".format(line_number))
                        ...
                        if user_id is not None and current_bio is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Ending bio body".format(line_number))
                    elif in_section['bio_body'] and current_bio is not None:
                        current_bio.append(line)
                        if verbose:
                            print(
                                "Line {0}: Adding to bio body: {1}".format(
                                    line_number, line))
                elif in_section['message_list'] and in_section['message_thread']:
                    if key == "Thread":
                        current_thread['Thread'] = validate_non_negative_integer(
                            value, "Thread", line_number)
                        if verbose:
                            print(
                                "Line {0}: Thread ID set to {1}".format(
                                    line_number, value))
                    elif key == "Category":
                        current_thread['Category'] = [
                            category.strip() for category in value.split(",")]
                        if verbose:
                            print(
                                "Line {0}: Category set to {1}".format(
                                    line_number, current_thread['Category']))
                    elif key == "Forum":
                        current_thread['Forum'] = [forum.strip()
                                                   for forum in value.split(",")]
                        if verbose:
                            print(
                                "Line {0}: Forum set to {1}".format(
                                    line_number, current_thread['Forum']))
                    elif key == "Title":
                        current_thread['Title'] = value
                        if verbose:
                            print(
                                "Line {0}: Title set to {1}".format(
                                    line_number, value))
                    elif key == "Type":
                        current_thread['Type'] = value
                        if verbose:
                            print(
                                "Line {0}: Type set to {1}".format(
                                    line_number, value))
                    elif key == "State":
                        current_thread['State'] = value
                        if verbose:
                            print(
                                "Line {0}: State set to {1}".format(
                                    line_number, value))
                    elif key == "Author":
                        current_message['Author'] = value
                        if verbose:
                            print(
                                "Line {0}: Author set to {1}".format(
                                    line_number, value))
                    elif key == "Time":
                        current_message['Time'] = value
                        if verbose:
                            print(
                                "Line {0}: Time set to {1}".format(
                                    line_number, value))
                    elif key == "Date":
                        current_message['Date'] = value
                        if verbose:
                            print(
                                "Line {0}: Date set to {1}".format(
                                    line_number, value))
                    elif key == "SubType":
                        current_message['SubType'] = value
                        if verbose:
                            print(
                                "Line {0}: SubType set to {1}".format(
                                    line_number, value))
                    elif key == "Post":
                        post_value = validate_non_negative_integer(
                            value, "Post", line_number)
                        ...
                        if verbose:
                            print(
                                "Line {0}: Post ID set to {1}".format(
                                    line_number, post_value))
                    elif key == "Nested":
                        nested_value = validate_non_negative_integer(
                            value, "Nested", line_number)
                        if nested_value != 0 and nested_value not in current_thread.get(
                                'post_ids', set()):
                            raise ValueError(
                                "Nested value '{0}' on line {1} does not match any existing Post values in the current thread. Existing Post IDs: {2}".format(
                                    nested_value, line_number, list(
                                        current_thread.get(
                                            'post_ids', set()))))
                        current_message['Nested'] = nested_value
                        if verbose:
                            print(
                                "Line {0}: Nested set to {1}".format(
                                    line_number, nested_value))
                        ...
                        if current_message is not None:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Starting message body".format(line_number))
                        ...
                        if current_message is not None and 'Message' in current_message:
                            ...
                            if verbose:
                                print(
                                    "Line {0}: Ending message body".format(line_number))
                        ...
                        if verbose:
                            print(
                                "Line {0}: Adding to message body: {1}".format(
                                    line_number, line))
    ...
    for service in services:
        ...
        for category in service['Categories']:
            ...
            print(
                "  Description: {0}".format(
                    category['Description'].strip().replace(
                        "\n", "\n    ")))
            print("")
        ...
        for idx, thread in enumerate(service['MessageThreads']):
            ...
            for message in thread['Messages']:
                print(
                    "    {0} ({1} on {2}): [{3}] Post ID: {4} Nested: {5}".format(
                        message['Author'],
                        message['Time'],
                        message['Date'],
                        message.get(
                            'SubType',
                            'Post' if message['Post'] == 1 or message['Nested'] == 0 else 'Reply'),
                        message['Post'],
                        message['Nested']))

                # Indent each line of the message body but keep it at the same
                # level
                print(
                    "      {0}".format(
                ...
                if 'Polls' in message and message['Polls']:
                    ...
                    for poll in message['Polls']:
                        ...
                        print(
                            "          Results: {0}".format(
                                ", ".join(
                                    str(r) for r in poll.get(
                                        'Results', []))))
                        print(
                            "          Percentage: {0}".format(
                                ", ".join(
                                    "{:.2f}".format(
                                        float(p)) for p in poll.get(
                                        'Percentage',
                                        []))))
                        print(
                            "          Votes: {0}".format(
                            ...
                            for poll in message['Polls']:
                                ...
                                output.append(
                                    "  Question: {0}".format(
                                        poll.get(
                                            'Question', 'N/A')))
                                output.append("  Answers: {0}".format(
                                    ", ".join(poll.get('Answers', []))))
                                output.append(
                                    "  Results: {0}".format(
                                        ", ".join(
                                            str(r) for r in poll.get(
...
def open_compressed_file(filename):
    ...
    if filename.endswith('.gz'):
        import gzip
        return gzip.open(
            filename, 'rt', encoding='utf-8') if not PY2 else gzip.open(filename, 'r')
    elif filename.endswith('.bz2'):
        import bz2
        return bz2.open(
            filename, 'rt', encoding='utf-8') if not PY2 else bz2.open(filename, 'r')
    elif filename.endswith('.xz') or filename.endswith('.lzma'):
        try:
            import lzma
        except ImportError:
            from backports import lzma
        return lzma.open(
            filename, 'rt', encoding='utf-8') if not PY2 else lzma.open(filename, 'r')
    else:
        return open(filename,
                    'r',
                    encoding='utf-8') if not PY2 else open(filename,
                                                           'r')
...
def add_category(
    ...
    if insub != 0:
        if not any(cat['ID'] == insub for cat in service['Categories']):
            raise ValueError(
                "InSub value '{0}' does not match any existing ID in service.".format(insub))


def add_message_thread(
        service,
        thread_id,
        title='',
        category='',
...
def add_message_post(
        ...
        message):
    thread = next(
        (t for t in service['MessageThreads'] if t['Thread'] == thread_id),
        None)
    if thread is not None:
        ...
        raise ValueError(
            "Thread ID {0} not found in service.".format(thread_id))
...
def add_poll(
    ...
    if thread is not None:
        message = next(
            (m for m in thread['Messages'] if m['Post'] == post_id), None)
        if message is not None:
            ...
            raise ValueError(
                "Post ID {0} not found in thread {1}.".format(
                    post_id, thread_id))
    else:
        raise ValueError(
            "Thread ID {0} not found in service.".format(thread_id))
...
def remove_category(service, category_id):
    category = next(
        (c for c in service['Categories'] if c['ID'] == category_id),
        None)
    if category:
        service['Categories'].remove(category)
    else:
        raise ValueError(
            "Category ID {0} not found in service.".format(category_id))
